...
def verify_transformers_version():
    ...
    if major < 4 or minor < 36:
        raise ValueError("the version transformers should greater or equal 4.36")
...
def load_args_from_checkpoint(args):
    ...
    # MoE relative
    args.num_experts = mixtral_args.get("num_local_experts", 8)
    args.moe_router_topk = mixtral_args.get("moe_router_topk", 2)
...
def set_mlp_state(args, layer, hf_layer):
    ...
    gate = mlp.block.moe_layer.gate.weight.weight.data.copy_
    hf_gate_weight = hf_mlp.gate.weight
    gate(hf_gate_weight)

    for idx in range(args.num_experts):
        w1 = getattr(mlp.block.moe_layer.experts.experts, f"{idx}").w1.weight.data.copy_
        w2 = getattr(mlp.block.moe_layer.experts.experts, f"{idx}").w2.weight.data.copy_
        w3 = getattr(mlp.block.moe_layer.experts.experts, f"{idx}").w3.weight.data.copy_
...
def _load_checkpoint(queue, args):
    ...
    md.moe_router_topk = getattr(margs, 'moe_router_topk', 2)
    ...
    for layer_num in range(margs.num_layers):
        ...
        mlp_gate_weight.append(layer.mlp.block.moe_layer.gate.weight.weight.data)

        if md.linear_bias:
            qkv_bias.append(layer.self_attention.query_key_value.bias.data)
            mlp_gate_bias.append(layer.mlp.block.moe_layer.gate.weight.bias.data)

        for expert_idx in range(margs.num_experts):
            message[f"mlp {expert_idx} w1 weight"] = getattr(
                layer.mlp.block.moe_layer.experts.experts, f"{expert_idx}").w1.weight.data
            message[f"mlp {expert_idx} w2 weight"] = getattr(
                layer.mlp.block.moe_layer.experts.experts, f"{expert_idx}").w2.weight.data
            message[f"mlp {expert_idx} w3 weight"] = getattr(
                layer.mlp.block.moe_layer.experts.experts, f"{expert_idx}").w3.weight.data
            if md.linear_bias:
                message[f"mlp {expert_idx} w1 bias"] = getattr(
                    layer.mlp.block.moe_layer.experts.experts, f"{expert_idx}").w1.bias.data
                message[f"mlp {expert_idx} w2 bias"] = getattr(
                    layer.mlp.block.moe_layer.experts.experts, f"{expert_idx}").w2.bias.data
                message[f"mlp {expert_idx} w3 bias"] = getattr(
                    layer.mlp.block.moe_layer.experts.experts, f"{expert_idx}").w3.bias.data
