...
def add_arguments(parser):
    group = parser.add_argument_group(title='ChatGLM3 HF loader.')
...
def load_args_from_checkpoint(args):
    ...
    if "multi_query_attention" in llama_args \
            and llama_args["num_attention_heads"] != llama_args["multi_query_group_num"] \
            and llama_args["multi_query_group_num"] != 1:
...
def set_attn_state(args, layer, hf_layer):
    ...
    qkv_weight = hf_attn.query_key_value.weight
    end_q = dim * nh
    end_k = end_q + ng * dim
    wq = qkv_weight[:end_q, :]
    wk = qkv_weight[end_q : end_k, :]
    wv = qkv_weight[end_k:, :]
    ...
    if args.add_qkv_bias:
        qkv_bias = hf_attn.query_key_value.bias
        bq = qkv_bias[:end_q]
        bk = qkv_bias[end_q : end_k]
        bv = qkv_bias[end_k:]
        attn.query_key_value.bias.data.copy_(torch.cat([
                bq.reshape((ng, dim * nh // ng)),
                bk.reshape((ng, dim)),
                bv.reshape((ng, dim)),
            ], dim=1).reshape((-1)))
        
    if args.add_dense_bias:
        attn.dense.bias.data.copy_(hf_attn.dense.bias)
...
def _load_checkpoint(queue, args):
    ...
    # ChatGLM3 requires HF transformers >=4.31.0.
