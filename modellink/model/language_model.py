...
from modellink.core.enums import LayerType, AttnMaskType, ModelType
from modellink.core.models.common.rotary_pos_embedding import RotaryEmbedding
from modellink.model.module import MegatronModule
from modellink.model.transformer import ParallelTransformer
from modellink.model.utils import get_linear_layer
from modellink.model.utils import init_method_normal, scaled_init_method_normal
...
def parallel_lm_logits(input_, word_embeddings_weight, parallel_output,
    ...
    if args.lm_norm_weight:
        word_embeddings_weight = torch.nn.functional.normalize(word_embeddings_weight)
...
class Embedding(MegatronModule):
    ...
    def __init__(self,
        ...
        self.add_position_embedding = args.position_embedding_type == PositionEmbeddingType.absolute
...
class TransformerLanguageModel(MegatronModule):
    ...
    def set_input_tensor(self, input_tensor):
        ...
        if self.add_encoder and self.add_decoder:
            ensure_valid(len(input_tensor) == 1,
                         'input_tensor should only be length 1 for stage with both encoder and decoder')
            self.encoder.set_input_tensor(input_tensor[0])
        elif self.add_encoder:
            ensure_valid(len(input_tensor) == 1, 'input_tensor should only be length 1 for stage with only encoder')
            self.encoder.set_input_tensor(input_tensor[0])
        ...
        if self.post_process:
            if self.add_pooler:
                ...
                self.pooler.load_state_dict(state_dict[self._pooler_key],
                                            strict=strict)
            if self.untie_embeddings_and_output_weights:
                ensure_valid('output_layer' in state_dict,
