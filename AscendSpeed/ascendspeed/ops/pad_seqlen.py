...
class PadSeqLenFunction(torch.autograd.Function):
    ...
    def forward(ctx, input_tensor, seqlen, maxseqlen):
        ...
        output_tensor = PadSeqLenFunction.ascendspeed_ops_pad.npu_pad_seqlen(input_tensor, seqlen, maxseqlen)
    ...
    def backward(ctx, input_tensor):
        ...
        """
        output_tensor = PadSeqLenFunction.ascendspeed_ops_unpad.npu_unpad_seqlen(input_tensor.contiguous(), ctx.seqlen, ctx.maxseqlen)
        return output_tensor, None, None

class PadSeqLen(torch.nn.Module):
    def __init__(self, maxseqlen):
        super(PadSeqLen, self).__init__()
        self.maxseqlen = maxseqlen

    def forward(self, input_tensor, seqlen):
        output_tensor = PadSeqLenFunction.apply(input_tensor, seqlen, self.maxseqlen)
        return output_tensor