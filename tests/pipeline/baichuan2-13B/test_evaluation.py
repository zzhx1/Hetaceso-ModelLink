...
class TestEvaluate(DistributedTest):
    ...
    @pytest.mark.parametrize("params", test_config["test_baichuan2_mmlu_evaluate"])
    def test_baichuan2_mmlu_evaluate(self, build_args, params):
        ...
        if dist.get_rank() == 0:
            print("=================== baichuan2 MMLU score ===============")
            print(log_capture)

            expected_score = acquire_score(log_capture)
            assert math.isclose(expected_score, 0.5790, abs_tol=1e-2), "forward pass has been changed, check it!"
